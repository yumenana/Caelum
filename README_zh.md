...
`GateTolerancePenalty` 对 `model.badi.last_gate` 在 GT 平坦区施加铰链惩罚，防止门控惰性导致浅层噪声泄漏。余弦退火至训练进度 70% 后自动归零，建议权重 λ≈0.001：
...
- [x] PPBUNet v1.2 架构设计（ParallelOAM · FrequencyRouter · MIM · RMA · HAT · CornerAwareDCN · AnimeCommitteeRefiner · SATUpsampler）
- [x] 多平台退化模拟 Pipeline（5 模式 · 三阶段高阶链路 · 在线实时生成）
- [x] 定制化损失函数体系（CaelumLossV2 · 13 子损失 · 两阶段渐进策略 · 委员会正交性 + BADI 门控正则）
- [x] 早期检查点效果验证
