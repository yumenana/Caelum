...
| **Architecture** | PPBUNet (U-Net + HAT Attention + Connected Manifold Wormhole) | SwinUNet / CNN | RRDB | U-Net | GLSL shader |
...
#### Real-world Web Image Reconstruction (no GT reference)

> The two sets below use crops grabbed directly from the wild internet — no original is available. They reflect what reconstruction quality actually feels like in real use (detail retention, cleanliness, artifact suppression).
...
| Stage | Function |
|-------|----------|
| **Shallow (RepSRBlock)** | Converts input to a shared latent space; multi-branch reparameterized at training, folded to a single Conv at inference (zero extra cost) |
| **ParallelOAM Bypass** | Full-resolution parallel branch with absolute 0°/90°/45°/135° directional basis; eliminates Manhattan staircase aliasing |
| **Encoder (2-Level)** | RCAB×2 hierarchical downsampling; progressively aggregates multi-scale spatial context |
| **FrequencyRouter** | Explicitly splits the latent into DC (flat color) and AC (edges / topology) streams |
| **HAT Bottleneck** | Window self-attention + OCAB; replaced PSMamba in v1.5 (2–3× training speedup, fully parallel) |
| **MIM + SimpleSkipFusion** | InfoNCE-purified skip filter, then concat → 1×1 → GroupNorm fusion; suppresses compressed-artifact propagation |
| **Decoder (HAT)** | Two-level HAT decoder; Level 0 is split into Pre / Post halves with CMW inserted between them |
| **ConnectedManifoldWormhole (CMW)** | Bilateral manifold wormhole; pulls global color prototypes from the DC branch and injects them at the mid-point of Decoder L0 |
| **CreviceAuxHead** | Training-only auxiliary supervision head; applies topology losses directly at the CMW output, zero inference cost |
| **Guard GroupNorm** | GroupNorm placed before TopologyGuidedDCN to cut off the 1e10-magnitude amplitude propagation that can otherwise build up at the decoder tail |
| **TopologyGuidedDCN** | Deformable convolution driven by ParallelOAM geometry priors; targeted 3–5px geometry refinement on hair tips and ≤4px crevices |
| **SATUpsampler v2** | Anisotropic dynamic filtering + implicit polygon rendering; singularity-aware ×4 upsampling |
...
- [x] PPBUNet v1.7 architecture design (ParallelOAM · FrequencyRouter · MIM+SimpleSkipFusion · HAT Bottleneck · ConnectedManifoldWormhole · CreviceAuxHead · TopologyGuidedDCN · SATUpsampler v2)
