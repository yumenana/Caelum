...
3. [核心模块](#3-核心模块)
   ...
   - 3.11 [基础锚定细节注入 (BaseAnchoredDetailInjector)](#311-基础锚定细节注入-baseanchoreddetailinjector)
   - 3.12 [各向异性流形感知上采样 (AMADSUpsampler)](#312-各向异性流形感知上采样-amadsupsampler)
4. [训练动力学](#4-训练动力学)
...
| 因果链: Geom→DCN→Palette→BADI→Up | `forward()` 末段 | 方向先验 → 曲率精修 → 色彩渲染 → 恒等锚定融合 → 上采样 |
...
| | 传统做法 | PPBUNet v1.0 | PPBUNet v1.1 (当前) |
|---|---------|---------|----------|
| **公式** | `out = network(x) + bicubic(x)` | `out = upsampler(feat_unet + x_shallow)` | `out = upsampler(BADI(feat_unet, x_shallow))` |
| **问题** | 网络被迫生成"反向噪声"抵消 bicubic 振铃，导致懒惰收敛 | 简单加法不加区分地注入伪影 | 恒等锚定保障梯度，空间门控精准注入高频细节 |
...
### 3.11 基础锚定细节注入 (BaseAnchoredDetailInjector)
...
> 🚨 **BADI 必须位于 PaletteModulation 之后、AMADSUpsampler 之前！**
> 色彩调制完成后的特征已是完整的重建结果，BADI 在此与 x_shallow 融合可以最大化利用深浅特征的互补信息。

### 3.12 各向异性流形感知上采样 (AMADSUpsampler)
...
| `losses.py` | CaelumLossV2 损失函数库 (12 子损失 + GateTolerancePenalty 可选) |
