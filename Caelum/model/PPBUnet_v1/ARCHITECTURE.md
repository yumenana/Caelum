...
3. [核心模块](#3-核心模块)
   ...
   - 3.11 [恒等锚定委员会精修器 (AnimeCommitteeRefiner)](#311-恒等锚定委员会精修器-animecommitteerefiner)
   - 3.12 [基础锚定细节注入 (BaseAnchoredDetailInjector)](#312-基础锚定细节注入-baseanchoreddetailinjector)
   - 3.13 [奇异点感知拓扑上采样 (SATUpsampler)](#313-奇异点感知拓扑上采样-satupsampler)
4. [训练动力学](#4-训练动力学)
...
| 决策 | 代码位置 | 原因 |
|------|----------|------|
| BADI 恒等锚定融合 | `badi(rendered_feat, x_shallow)` | 恒等锚定保障深层梯度全程畅通，空间门控按结构决定细节注入量 |
...
| 因果链: Geom→DCN→Palette→CommitteeRefiner→BADI→Up | `forward()` 末段 | 方向先验 → 曲率精修 → 色彩渲染 → 委员会细节精修 → 恒等锚定融合 → 上采样 |
...
| | 传统做法 | PPBUNet v1.0 | PPBUNet v1.2 (当前) |
|---|---------|---------|----------|
| **公式** | `out = network(x) + bicubic(x)` | `out = upsampler(feat_unet + x_shallow)` | `out = SATUp(CommitteeRefiner(BADI(feat_unet, x_shallow)))` |
| **问题** | 网络被迫生成"反向噪声"抵消 bicubic 振铃，导致懒惰收敛 | 简单加法不加区分地注入伪影 | 恒等锚定保障梯度，委员会精修补充细节，奇异点感知上采样保护线条拓扑 |
...
**v1.2 数值稳定性改进：**
- `log_map` 中 `acos` 替换为 `atan2(v_norm, inner)`：`acos` 在 inner→±1 时导数趋于无穷，AMP float16 下引发梯度爆炸；`atan2` 在全域梯度有界
- `exp_map` 中 `torch.where` 替换为 `v_norm.clamp(min=eps)`：`torch.where` 不阻断梯度流，被屏蔽分支的 inf/nan 梯度仍会反向传播污染 AMP 权重
- eps: 1e-5 → 1e-4，提升低精度训练鲁棒性
...
2. **DensePathExtractor (RDB 密集块)**: 联合特征与角点先验，经三层密集残差块提取线条路径上下文
3. 路径特征 → 18 个坐标偏移 + 9 个注意力掩码
4. DCNv2 在变形网格上执行 3×3 卷积，采样点收缩包裹尖端
...
### 3.11 恒等锚定委员会精修器 (AnimeCommitteeRefiner)

在 PaletteModulation 输出进入 BADI 之前，以**恒等锁定**的方式注入四个层次小修正路径。
...
|------|------|----------|
| Colorist | 1×1 | 全局色彩平衡 |
| Inker | DW 3×3 | 线条锡化与童区层次 |
| Polisher | Dilated 3×3 (d=2) | 中等纹理光滑 |
| Detailer | Group 3×3 | 屈1米高频纹理补就 |

**动态路由：**共享轻量 router (1×1 Conv → Softmax)，输出各委员对应的加权 $w_i$。

$$\text{Out} = \text{feat} + \sum_{i=1}^{4} w_i \cdot \text{Expert}_i(\text{feat})$$

所有 Expert 最后一层零初始化，训练初期各路径输出≈±0，模型与知识蚸馏前等价。Router bias=-3.0（sigmoid≈0.05）防止训练初期期修正增幅过大干扰基础梯度流。

**正交性正则化：**配合 `CommitteeOrthogonalityLoss` 驱动四张路由捺趋空间正交，突出各委员的分工専业化。

**监控接口：**`model.committee_refiner.last_gate_sum`，训练时可观察四别就率分布。

### 3.12 基础锚定细节注入 (BaseAnchoredDetailInjector)

替代早期简单残差相加 `rendered_feat + x_shallow`。

**vs 简单残差相加**：`x_shallow` 来自退化输入的 RepSRBlock 特征，携带 JPEG 块效应、色带、伪色等压缩伪影。简单加法在最后一步重新注入退化信号，迫使 U-Net 全链路的净化努力功亏一篑。

**第一性原理重构 — 恒等锚定法则 (Identity-Anchored)：**

1. **承重墙直通**: 深层纯净特征 `feat_deep`（PaletteModulation 输出）以 100% 权重直接参与加法，永远受到完整重建损失约束，彻底杜绝梯度断头台
2. **跨层细节萃取**: `detail_extractor(cat[feat_deep, feat_shallow])` 联合双层上下文提取高频修补残差（最后一层零初始化，训练初期残差≈0）
3. **基底主导门控**: `spatial_gate(feat_deep)` 由深层语义决定注入位置，线稿区激活、平坦噪点区趋零（bias=-3.0 → sigmoid≈0.05，透明启动）

公式：`Out = feat_deep + sigmoid(spatial_gate(feat_deep)) × detail_extractor(cat[feat_deep, feat_shallow])`

训练时通过 `model.badi.last_gate`、`last_gate_mean`、`last_detail_mag` 监控门控状态。

**可选辅助损失 `GateTolerancePenalty`**：在 GT 平坦区施加铰链惩罚（建议 λ≈0.001），防止门控惰性导致浅层噪声泄漏。余弦退火至训练进度 70% 后自动归零。

> 🚨 **BADI 必须位于 PaletteModulation 之后、SATUpsampler 之前！**
> 色彩调制完成后的特征已是完整的重建结果，BADI 在此与 x_shallow 融合可以最大化利用深浅特征的互补信息。

### 3.13 奇异点感知拓扑上采样 (SATUpsampler)

| 特性 | SATUpsampler | 旧版 AMADSUpsampler |
|------|-------------|--------------------|
| 核心机制 | 5×5 动态滤波核（einsum）+ PDE 对流注入 | 全卷积几何 → 坐标偏移 → grid_sample |
| 空间连续性 | ✅ 卷积天生局部连续 | ✅ grid_sample 连续 |
| 各向异性 | ✅ 每像素独立生成 5×5核 + 方向强度调制对流 | ✅ 边缘门控 α(x) |
| 拓扑保持 | ✅ PDE 对流等高线传输减少奇异点漫流 | ❌ 无拓扑约束 |
| DC 保护 | ✅ Softmax 归一化 (kernel 和 = 1) | ✅ 零初始化安全 |

**动态滤波核：**

对每个像素位置独立生成一个 5×5 滤波核，通过 einsum 执行内容感知卷积：

$$\text{out}[b,c,s] = \sum_{k=1}^{25} K[b,c,k] \cdot \text{x\_pad}[b,c,k,s],\quad K=\text{Softmax}(\text{kernel\_gen}(f))$$

- `kernel_gen`: 1×1 投影 + DW 3×3 内容感知 + 1×1 投影至 $C\times25$
- Softmax 在 $k$ 维度归一化：每个输出通道的滤波和恒为 1，消除 DC 漂移

**PDE 对流注入：**

基于波动方程 $\partial I/\partial t + v_x \partial I/\partial x + v_y \partial I/\partial y = 0$ 提取各向异性修正项：

$$\Delta = v_x \cdot \nabla I_x + v_y \cdot \nabla I_y, \quad \text{Out} = K*I + \gamma \cdot \Delta$$

- `velocity_gen`: 零初始化 bias（训练初期 $v\approx0$，等价单纯动态滤波）
- `advection_gamma`: 可学习标量 (init=0.1)，控制对流强度
- Scharr 梯度缓冲预展开至通道数，避免广播开销

**尺度统一界面：**`AdvancedUpsampler` 封装多次上采样 (2×/4×/8×) 递归拆分为 2×阶段，每阶段一个 `SATUpsampler` 实例，不同倍数共享模组。
...
**BADI 门控正则**：`GateTolerancePenalty` 对 `model.badi.last_gate` 在 GT 平坦区施加铰链惩罚，建议 λ≈0.001。训练进度达 70% 后余弦退火至 0，可选使用。
**委员会正交性正则**：`CommitteeOrthogonalityLoss` 驱动 AnimeCommitteeRefiner 四张路由捺趋空间正交，建议 λ=0.1。

**损失函数我映 (CaelumLossV2, v1.2)**：

| 键 | 类 | 监管内容 | 默认权重 |
|-----|-----|---------|--------|
| `oklch` | 感知色彩 | OKLab 圆柱坐标色差 | 5.0 |
| `dc` | DC 锁定 | Scharr 能量指数衰减软权重 | 1.0 |
| `stgv` | 全变分 | 严格平坦-TV + 二阶平滑 | 1.0 |
| `chroma_grad` | 色彩梯度 | 色度梯度对齐 | 1.5 |
| `crevice` | 拓扑 | 岁差与 ZNCC 高频 | 6.0 |
| `histogram` | 色彩分布 | 差分直方图匹配 | 1.5 |
| `gibbs` | Gibbs | 达林盘利-驾马振铃级差 | 4.0 |
| `smooth_grad_hessian` | 平滑度 | Hessian 蚌度约束 | 1.5 |
| `angular` | 角度 | 梯度方向平滑度 | 5.0 |
| `turning_point` | 局部完美度 | 宏观上采样(张量角点响应) | 0.5 |
| `lrt` | 拓扑 | 拉普拉西谐振拓扑 + Charbonnier | 1.5 |
| `ridge` | 脉山 | 脉山连续性与方向性 | 0.5 |
| `perceptual` | 感知 | VGG 层级+频谱圆型感知 | 0.5 |

**可选展局捯失：**
- `GateTolerancePenalty` (λ≈0.001) — BADI 门控平坦区正则
- `CommitteeOrthogonalityLoss` (λ=0.1) — 委员会路由掩码正交性
...
| `losses.py` | CaelumLossV2 捯失函数库 (13 子捯失 + CommitteeOrthogonalityLoss + GateTolerancePenalty 可选) |
